...
defmodule Astarte.DataAccess.Data do
  ...
  def fetch_property(
    ...
    property = Repo.fetch_by(query, data_id, consistency: :quorum, error: :property_not_set)

    with {:ok, %{^value_column => value}} <- property do
      case value do
  ...
  def path_exists?(
      ...
      when is_binary(device_id) and is_binary(path) do
    keyspace = Realm.keyspace_name(realm)
    data_id = data_id(device_id, interface_descriptor, mapping, path)

    from(IndividualProperty, prefix: ^keyspace, where: ^data_id)
  ...
  def fetch_last_path_update(
    ...
    with {:ok, property} <- Repo.fetch_by(query, data_id, error: :path_not_set) do
  ...
  defp data_id(
    ...
    data_id(device_id, interface_id, endpoint_id, path)
  end

  defp data_id(device_id, interface_id, endpoint_id, path) do
